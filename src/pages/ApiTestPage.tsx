...
import { getMockAuthorizationHeader, type MockTokenMode } from "../mocks/msw/utils/auth";
...
const resolveUrl = (url: string) => (url.startsWith("/api/") ? `${window.location.origin}${url}` : url);
...
function ApiTestPage() {
    ...
    const handleMethodChange = (nextMethod: HttpMethod) => {
        ...
        setUrl(nextMethod === "GET" ? "/api/v1/users/me/bread-collections" : nextMethod === "POST" ? "/users" : "/users/1");
    };
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            setLatestLog({
                ...
                response: isAxiosError(error) ? (error.response?.data ?? error.message) : "Request failed",
            });
        }
    };
            ...
            <section className="mt-6 grid gap-6 lg:grid-cols-[24rem_1fr]">
                <form className="flex flex-col gap-4 rounded-lg border border-slate-200 p-5" onSubmit={handleSubmit}>
                    <div className="grid gap-2">
                        ...
                        <select className="h-10 rounded-md border border-slate-300 px-3" id="method" value={method} onChange={(event) => handleMethodChange(event.target.value as HttpMethod)}>
                            {methods.map((item) => (
                    ...
                    <div className="grid gap-2">
                        ...
                        <select className="h-10 rounded-md border border-slate-300 px-3" id="tokenMode" value={tokenMode} onChange={(event) => setTokenMode(event.target.value as MockTokenMode)}>
                ...
                <div className="rounded-lg border border-slate-200 p-5">
                    ...
                    <div className="mt-5 grid gap-5">
                        ...
                        <div>
                            ...
                            <pre className="mt-2 min-h-24 max-h-80 overflow-auto rounded-md bg-slate-900 p-4 font-mono text-sm whitespace-pre-wrap text-slate-100">

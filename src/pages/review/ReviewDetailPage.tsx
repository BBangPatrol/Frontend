...
export default function ReviewDetailPage() {
  ...
  if (queryError) {
    const message = isAxiosError<StoreApiErrorResponse>(queryError)
      ? queryError.response?.data.message
      : undefined;
    return (
      <PageStatus
        message={message ?? "리뷰를 불러오지 못했습니다."}
        showBackButton
      />
    );
  }

  if (storeDetailQuery.isPending || storeReviewsQuery.isPending)
    return <PageStatus message="리뷰를 불러오는 중입니다." isLoading />;
  ...
  return (
    <main className="p-4 flex flex-col gap-5 md:mx-auto md:w-4xl md:px-6 md:py-8 md:gap-6 md:justify-start md:items-start">
      <PageHeader
        ...
        subTitle={
          <>
            총 <span className="text-black-01">{count.toLocaleString()}</span>
            개의 리뷰가 있습니다.
          </>
        }
      />

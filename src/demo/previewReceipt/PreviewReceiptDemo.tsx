...
export default function PreviewReceiptDemo() {
  ...
  const copyImageUrl = async () => {
    ...
    try {
      await navigator.clipboard.writeText(previewMutation.data.imageUrl);
  ...
  return (
    <>
      <button
        ...
        className="bg-white border border-sub-01 rounded-xl py-3 w-40"
      >
        시연용 사진 발급
      </button>
        ...
        >
          <div className="relative max-h-[90dvh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-6 shadow-dropdown md:p-8">
            <button
              ...
              onClick={closeModal}
              aria-label="닫기"
              className="absolute top-4 right-5 text-2xl leading-none text-gray-02"
            >
              ×
            </button>
            ...
            <div className="flex flex-col gap-6">
              <header className="pr-8">
                <h2 id="preview-receipt-title" className="typo-head-03">시연용 사진 발급</h2>
                <p className="mt-2 text-gray-02 typo-body-04">시연 코드를 입력하면 영수증 사진을 발급합니다.</p>
              </header>

              <form onSubmit={submitCode} className="flex flex-col gap-3">
                <label htmlFor="preview-receipt-code" className="typo-head-05">시연 코드</label>
                <input
                ...
                <button
                  ...
                  className="w-full rounded-xl bg-sub-01 py-3 text-white typo-head-05 disabled:opacity-50"
                >
                  {previewMutation.isPending ? "발급 중..." : "사진 발급하기"}
                </button>
                {previewMutation.isError && (
                  <p role="alert" className="text-center text-red typo-body-04">{getErrorMessage(previewMutation.error)}</p>
                )}
              </form>
                ...
                <div className="flex flex-col gap-4 border-t border-gray-04 pt-6">
                  <div className="flex w-full items-center justify-center overflow-hidden rounded-2xl border border-gray-04 bg-gray-04 p-3">
                    <img
                      ...
                      alt="발급된 시연용 영수증"
                      className="block h-48 w-40 object-contain md:h-56 md:w-48"
                    />
                  </div>
                  ...
                  <dl className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <dt className="text-gray-02 typo-body-04">이미지 URL</dt>
                      <dd className="rounded-xl bg-gray-04 p-3 break-all typo-body-04">{previewMutation.data.imageUrl}</dd>
                    </div>
                  </dl>
                  ...
                  <button
                    ...
                    className="w-full rounded-xl border border-sub-01 py-3 text-sub-01 typo-head-05"
                  >
                    {copyStatus === "success" ? "URL 복사 완료" : "이미지 URL 복사"}
                  </button>
                  {copyStatus === "error" && <p role="alert" className="text-center text-red typo-body-04">URL을 복사하지 못했습니다.</p>}
                </div>
              )}
            </div>
          </div>

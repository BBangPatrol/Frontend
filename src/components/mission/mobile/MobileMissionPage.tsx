...
export default function MobileMissionPage({ missions }: { missions: MissionProgress[] }) {
  return (
    ...
    <div className={`p-4 flex flex-col gap-4 ${isSuccess ? "bg-green-50/30" : "bg-white"} rounded-2xl border-2 ${isSuccess ? "border-green-500/20" : "border-gray-04"}`}>
      <div className="flex justify-between">
        <div className="flex flex-col gap-1">
          <h3 className="text-black-01 typo-head-04">중구 마스터</h3>
          <p className="text-gray-02 typo-body-04">대전 중구의 빵집 5곳 방문하기</p>
        </div>
        <div className="flex p-1 gap-1 bg-gray-04 rounded-lg self-start">
          <div>
            <img src={pointIcon} />
          </div>
          <p className="text-gray-01 typo-body-04">100P 적립{isSuccess && " 완료!"}</p>
        </div>
      </div>
      <div className="flex flex-col gap-1.5">
        <div className="flex justify-between">
          <p className={`${isSuccess ? "text-green" : "text-sub-01"} typo-sub-02`}>진행률</p>
          <p className="text-gray-01 typo-body-04">

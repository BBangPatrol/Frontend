...
export default function DesktopReviewDetailPage({ ratingPercentages, sort, onSortChange }: DesktopReviewDetailPageProps) {
  return (
    <main className="mx-auto w-4xl px-6 py-8 flex flex-col justify-start items-start gap-6">
      <PageHeader
        title="성심당 본점 리뷰"
        subTitle={
          <>
  ...
  return (
    <section className="bg-yellow-02 flex p-6 gap-8 w-full rounded-3xl border border-main-05">
      <div className="flex flex-col justify-center items-center pl-12 pr-14">
        ...
        <p className="text-gray-02 typo-body-03">1,234명 참여</p>
      </div>
        ...
        {ratingPercentages.map(([rating, value]) => {
          ...
          return (
            <div key={rating} className="flex gap-4 w-full items-center">
              <p className="w-6 text-gray-01 typo-body-03">{rating}점</p>
              ...
              <p className="w-8.5 text-right text-gray-02 typo-body-03">{value}%</p>
            </div>
          );
        })}
      </div>

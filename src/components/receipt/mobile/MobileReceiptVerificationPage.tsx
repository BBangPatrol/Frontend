...
export default function MobileReceiptVerificationPage({ isAnalyzing, onAnalyze }: MobileReceiptVerificationPageProps) {
  if (isAnalyzing) return <AnalyzeModal />;

  return (
    <main className="mx-auto p-4 flex flex-col gap-10">
...
function ReceiptUpload({ onAnalyze }: { onAnalyze: () => void }) {
  return (
    <section className="flex flex-col items-center gap-8 p-7 border-2 rounded-4xl border-dashed border-main-02">
      ...
      <p className="text-gray-01 font-extrabold">영수증 촬영 또는 업로드</p>
      <div className="flex flex-col gap-2 w-full">
        <button onClick={onAnalyze} className="py-3 bg-[#C68B59] rounded-xl text-white typo-head-05">
          카메라
        </button>
  ...
  return (
    <section className="p-4 flex flex-col gap-3 bg-yellow-01 rounded-2xl">
      <div className="flex gap-1">
        <img src={cameraIcon} />
        <h3 className="text-black-01 typo-head-05">이렇게 찍어주세요!</h3>
      </div>
      <ul className="flex flex-col gap-2">
        <li className="flex gap-1">
          <img src={checkIcon} />
          <p className="text-gray-01 typo-sub-02">매장 이름, 결제 일시, 결제 금액이 명확해야 합니다.</p>
        </li>
        <li className="flex gap-1">
          <img src={checkIcon} />
          <p className="text-gray-01 typo-sub-02">화면이나 종이가 구겨지지 않게 평평하게 해서 찍어주세요.</p>
        </li>
        <li className="flex gap-1">
          <img src={crossIcon} />
          <p className="text-gray-01 typo-sub-02">카드 번호 등 민감한 개인정보는 가리고 올려주세요.</p>
        </li>
      </ul>
    </section>
  );

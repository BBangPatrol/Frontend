...
function DrawResultItemImage({ result }: { result: DrawCollectibleResult }) {
  return (
    <div className="relative flex justify-center items-center">
      ...
      <img className="size-40 z-10 object-contain" src={result.image} alt={result.name} />
    </div>
  );
}
...
function DrawResultExplain({ result }: { result: DrawCollectibleResult }) {
  ...
  return (
    <div className="flex flex-col gap-3 items-center">
      <p className={`px-2 py-1 rounded-2xl typo-body-04 md:text-sm! ${rankStyle.badge}`}>희귀도: {rankStyle.label}</p>
      ...
      <div className="text-center text-gray-02 typo-sub-02 md:text-sm!">
        {result.duplicated ? <p>중복 수집품으로 {result.refundPoint}P가 반환되었습니다.</p> : <p>새로운 수집품을 획득했습니다!</p>}
...
function DrawResultButtons({ onClose }: { onClose: () => void }) {
  return (
    <div className="flex flex-col md:flex-row gap-2 md:gap-3">
      <Link to="/collection" className="py-3 text-center bg-sub-01 rounded-xl shadow-[0px_4px_6px_-1px_rgba(198,139,89,0.20)] text-white typo-head-05 md:order-2 md:flex-1">
        도감 확인
      </Link>
